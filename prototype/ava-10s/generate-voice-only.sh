#!/usr/bin/env bash
# Generates the 10-second Ava prototype: voice only, no face/character.
#
...
OUTPUT_PATH="${1:-./output/ava-10s-voice-only.mp4}"
WORKDIR="$(mktemp -d)"
trap 'rm -rf "$WORKDIR"' EXIT

SCRIPT_LINE="I'm Ava. I'm 23, I have zero dollars, and I'm going to build a billion-dollar company."
...
echo "4/5 Padding audio to 10s (0.6s lead-in + trailing silence)..."
ffmpeg -y -loglevel error -i "$WORKDIR/voice.wav" \
  -af "adelay=600|600,apad" -t 10 -ar 44100 -ac 2 "$WORKDIR/padded.wav"
...
ffmpeg -y -loglevel error \
  -f lavfi -i "color=c=0x0a0a0a:s=1080x1920:d=10:r=30" \
  -i "$WORKDIR/padded.wav" \
  -filter_complex "
    [1:a]showwaves=s=900x260:mode=cline:colors=0x34d399|0x10b981:rate=30:scale=sqrt[wave];
    [0:v][wave]overlay=(W-w)/2:1120:format=auto[bg1];
    [bg1]drawtext=fontfile=${FONT}:textfile=${WORKDIR}/title.txt:fontcolor=0xf5f5f5:fontsize=54:x=(w-text_w)/2:y=260:line_spacing=10[bg2];
    [bg2]drawtext=fontfile=${FONT_REG}:textfile=${WORKDIR}/caption.txt:fontcolor=0xe5e5e5:fontsize=50:x=(w-text_w)/2:y=1500:line_spacing=16:enable='between(t,0.55,9.7)'[bg3];
    [bg3]drawtext=fontfile=${FONT_REG}:textfile=${WORKDIR}/disclosure.txt:fontcolor=0x9ca3af:fontsize=26:x=(w-text_w)/2:y=1800[outv]
  " \
  -map "[outv]" -map 1:a \
